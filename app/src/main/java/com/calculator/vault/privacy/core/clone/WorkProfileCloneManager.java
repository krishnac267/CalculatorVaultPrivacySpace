...
public final class WorkProfileCloneManager {
    ...
    public CloneSpaceStatus getStatus() {
        ...
        if (shouldUseSamsungDualApps()) {
            StringBuilder message = new StringBuilder(
                    "Samsung Knox blocks in-app Clone Space on this phone. "
                            + "Use Samsung's built-in cloning instead, then tap Add App → Clone here."
            );
            if (samsungDualAppHelper.isDualMessengerAvailable()) {
                message.append(" Dual Messenger: Settings → Advanced features → Dual Messenger.");
            }
            if (samsungDualAppHelper.isSecureFolderAvailable()) {
                message.append(" Secure Folder: Settings → Security and privacy → Secure Folder.");
            }
            return new CloneSpaceStatus(
                    true,
                    false,
    ...
    public boolean shouldUseSamsungDualApps() {
        return samsungDualAppHelper.isSamsungDevice() && findWorkProfileUser() == null;
    }

    public boolean isSamsungSecureFolderAvailable() {
        return samsungDualAppHelper.isSecureFolderAvailable();
    }
    ...
    public Intent buildSamsungSecureFolderIntent() {
        return samsungDualAppHelper.buildSecureFolderIntent();
    }
